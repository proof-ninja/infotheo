...
case: ifPn => [/bigfcupP[dC] | Hc].
  ...
  rewrite [X in (_ + X)%coqR = _](_ : _ = 0) ?addR0; last first.
  ...
  move=> b1 fa0b1.
  have [-> _|gb1c] := eqVneq (g b1 c) 0%coqR; first by rewrite mulR0.
...
- move=> a1 Ha1.
  have [-> | ga1c] := eqVneq (g a1 c) 0%coqR; first by rewrite mulR0.
  have [-> | fa0a1] := eqVneq (f a0 a1) 0%coqR; first by rewrite mul0R.
  case/bigfcupP : Hc.
...
- apply/esym/psumR_seq_eq0P=>// a1 Ha1; first by apply mulR_ge0.
...
case: ifPn => [/bigfcupP[dB] | Hb0].
  ...
  rewrite big_split /= -2!big_distrr /=; congr (_ * _ + _ * _)%coqR.
